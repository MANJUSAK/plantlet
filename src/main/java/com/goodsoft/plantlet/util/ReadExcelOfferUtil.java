...
/**
 * function 苗木造价excel超大数据读取工具类
 * Created by 严彬荣 on 2017/9/30.
 * version v1.0
 */
@SuppressWarnings("ALL")
public class ReadExcelOfferUtil extends DefaultHandler {
    /**
     * 创建ExcelUtil类的单例（详情见本包下UUIDUtil类） start
     **/
    private volatile static ReadExcelOfferUtil instance;

    private ReadExcelOfferUtil() {
    }

    public static ReadExcelOfferUtil getInstance() {
        if (instance == null) {
            synchronized (ReadExcelOfferUtil.class) {
                if (instance == null)
                    instance = new ReadExcelOfferUtil();
            }
        }
        return instance;
    }
    ...
    public List<SeedlingOffer> list = new ArrayList<SeedlingOffer>();
    private String str = "";
    // 当前行
        ...
        for (int i = 0, len = rowList.size(); i < len; ++i) {
            ...
            switch (i) {
                case 0:
                    ...
                    break;
                case 1:
                    sd.setSdName(this.str);
                    break;
                case 2:
                    if (this.str != "") {
                        AnalysisParam var = null;
                        try {
                            var = this.analysisUtil.getSpecAnalysis(this.str);
                ...
                case 5:
                    if (this.str != "") {
                        try {
                            ...
                            int yera = Integer.parseInt(new SimpleDateFormat("yyyy").format(data));
                            int month = Integer.parseInt(new SimpleDateFormat("MM").format(data));
                            sd.setYear(yera);
                            sd.setMonth(month);
                        } catch (Exception e) {
                        }
    ...
    public boolean equals(Object o) {
        ...
        ReadExcelOfferUtil that = (ReadExcelOfferUtil) o;
